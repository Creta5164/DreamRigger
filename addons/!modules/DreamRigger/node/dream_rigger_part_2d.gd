## A [Node2D]-based rigging component that manages character parts through pose and sprite manipulation.[br]
...
## Emitted when the [member DreamRiggerPart2D.pose] property is modified.
signal pose_changed(pose: DreamRiggerPose)

## Emitted when the assigned [member DreamRiggerPart2D.pose] resource is modified.
signal pose_modified(pose: DreamRiggerPose)

## Emitted when the [member DreamRiggerPart2D.sprite] property is modified.
signal sprite_changed(sprite: DreamRiggerSprite)

## Emitted when the [member DreamRiggerPart2D.flip_h] or [member DreamRiggerPart2D.flip_v] property is modified.
signal flip_state_changed(h: bool, v: bool)

## Emitted when [member DreamRiggerPart2D.position_offset] value changed.
signal position_offset_changed(offset: Vector3)

## Emitted when [member DreamRiggerPart2D.rotation] value changed.
...
## Integer uid for selecting the current pose from the [member DreamRiggerPart2D.pose] resource.
@export
var pose_uid: int:
    get:
...
## current [member DreamRiggerPart2D.sprite].[br]
...
static func find_parent_part(node: Node, find_root: bool = true, topmost_node: Node = null) -> DreamRiggerPart2D:
...
## Updates joint positions for all child nodes in the rig hierarchy based on current [member DreamRiggerPart2D.sprite] data.
...
func resolve_child_position(child_node: Node = null) -> void:
    ...
    #조인트가 없고, 노드가 DreamRiggerPart2D가 아니라면 제어하지 않음.
    elif !has_joint:
        return
    ...
    resolved_position = (
        sprite.to_sprite_space(resolved_position)
      * Vector3(scale.x, scale.y, 1)
    )
