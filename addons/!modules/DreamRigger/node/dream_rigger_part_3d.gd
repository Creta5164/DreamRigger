## A [Node3D]-based rigging component that manages character parts through pose and sprite manipulation.[br]
## Handles hierarchical positioning, joint management, and sprite rendering for modular character animation systems.
@tool
class_name DreamRiggerPart3D extends Sprite3D
...
## Emitted when the [member DreamRiggerPart3D.pose] property is modified.
signal pose_changed(pose: DreamRiggerPose)

## Emitted when the assigned [member DreamRiggerPart3D.pose] resource is modified.
signal pose_modified(pose: DreamRiggerPose)

## Emitted when the [member DreamRiggerPart3D.sprite] property is modified.
signal sprite_changed(sprite: DreamRiggerSprite)

## Emitted when the [member DreamRiggerPart3D.flip_h] or [member DreamRiggerPart3D.flip_v] property is modified.
signal flip_state_changed(h: bool, v: bool)

## Emitted when [member DreamRiggerPart3D.position_offset] value changed.
signal position_offset_changed(offset: Vector3)

## Emitted when [member DreamRiggerPart3D.rotation] value changed.
...
## Integer uid for selecting the current pose from the [member DreamRiggerPart3D.pose] resource.
@export
var pose_uid: int:
    get:
...
## It's not bound with [member Node3D.transform], it'll be set to this z value when position resolved.
@export
...
## current [member DreamRiggerPart3D.sprite].[br]
## Key : [StringName] | Value : [DreamRiggerJoint]
...
## An resolved relative Z position from root part.
...
func _init() -> void:
    ...
    self.centered    = true
    self.axis        = Vector3.AXIS_Z
    self.transparent = true
    self.shaded      = true
    
    self.alpha_cut               = SpriteBase3D.ALPHA_CUT_DISCARD
    self.alpha_scissor_threshold = 0.01
    self.alpha_hash_scale        = 1.0
    self.alpha_antialiasing_mode = BaseMaterial3D.ALPHA_ANTIALIASING_ALPHA_TO_COVERAGE_AND_TO_ONE
    self.alpha_antialiasing_edge = 0.5
    pass
...
func _on_sprite_changed() -> void:
    ...
    self.pixel_size = sprite.pixels_to_units
...
static func find_parent_part(node: Node, find_root: bool = true, topmost_node: Node = null) -> DreamRiggerPart3D:
...
## Finds parent or root part.
static func find_all_family_parts(part: DreamRiggerPart3D) -> Array[DreamRiggerPart3D]:
    
    if !is_instance_valid(part):
        return []
    
    var family_parts: Array[DreamRiggerPart3D]
    
    var root_part := find_parent_part(part)
...
static func _iterate_get_child_parts(part: DreamRiggerPart3D, parts: Array[DreamRiggerPart3D]) -> void:
...
func resolve_positions() -> void:
    ...
    rotation.z = resolved_rotation_z
    ...
    if is_instance_valid(sprite):
        ...
        self.position = Vector3(0, 0, sprite.to_sprite_space(offset).z)
...
## Updates joint positions for all child nodes in the rig hierarchy based on current [member DreamRiggerPart3D.sprite] data.
...
func resolve_child_position(child_node: Node = null) -> void:
    ...
    if child_node is DreamRiggerPart3D:
    ...
    #조인트가 없고, 노드가 DreamRiggerPart3D가 아니라면 제어하지 않음.
    elif !has_joint:
        return
    ...
    resolved_position = sprite.to_sprite_space(resolved_position) * scale
    ...
    child_node.position = resolved_position
