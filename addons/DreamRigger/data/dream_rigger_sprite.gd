...
## Extended signal of [signal Resource.changed] that comes with sender.
signal changed_ext(sender: DreamRiggerSprite)
...
## Float value determining how many pixels represent one world unit.
## [br]
...
## Read-only float value representing the conversion scale from pixels to world units.
## [br]
## Calculated as [code]1.0 / pixels_per_unit[/code] or [code]1.0[/code]
## if [member pixels_per_unit] is [code]0[/code] or negative.
...
func _init() -> void:
    ...
    changed.connect(changed_ext.emit.bind(self))
...
## Returns joint by given [param name], returns [code]null[/code] if not exist.
...
## Converts a position from world space to sprite space using [member pixels_to_units].
## [br]
...
## Converts a position from sprite space to world space using [member pixels_to_units].
